...
* **문제:** YOLOv8의 기본 증강 설정 중, `shear`나 `hsv` 값이 높게 설정되어 있어, 차량량의 적재함 라인이 왜곡되거나 과적 여부를 판단하는 미세한 특징이 사라지는 현상 발생.
...
* **튜닝 상세:**
  ...
  * **Degrees (0.0):** 차량은 주로 수평으로 주행하므로, 불필요한 회전을 0으로 고정하여 학습 혼란 
